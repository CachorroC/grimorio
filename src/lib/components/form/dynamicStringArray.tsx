...
export default function DynamicStringArray({ title, items, onChange }: DynamicStringArrayProps) {
  ...
  const handleRemove = (indexToRemove: number) => {
    onChange(safeItems.filter((_, index) => index !== indexToRemove));
  };
  ...
  return (
    <div className={styles.section}>
      ...
      {safeItems.map((item, index) => (
        <div key={`${title}-${index}`} className={styles.arrayItem}>
          <input
            type="text"
