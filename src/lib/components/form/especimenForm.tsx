...
export default function EspecimenForm({
  ...
  const [includeImagenes, setIncludeImagenes] = useState<boolean>(() => {
  ...
  const [formData, setFormData] = useState<EspecimenType>(() => {
    ...
    return {
      ...
      polaridadEnergetica: initialData.polaridadEnergetica || ['Feminine'],
  ...
  const updateFormDataField = (field: keyof EspecimenType, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleSubmit: SubmitEventHandler<HTMLFormElement> = async (e) => {
    ...
    const payloadToSave: EspecimenType = {
      ...formData,
      nombresComunes: (formData.nombresComunes || []).filter((val) => val.trim() !== ''),
      partesUtiles: (formData.partesUtiles || []).filter((val) => val.trim() !== ''),
      esenciasFlorales: (formData.esenciasFlorales || []).filter((val) => val.trim() !== ''),
      propiedadesMedicinales: (formData.propiedadesMedicinales || []).filter((val) => val.trim() !== ''),
      correspondenciasEnergeticas: (formData.correspondenciasEnergeticas || []).filter((val) => val.trim() !== ''),
      malesEmocionales: (formData.malesEmocionales || []).filter((val) => val.trim() !== ''),
      malesFisicos: (formData.malesFisicos || []).filter((val) => val.trim() !== ''),
      taxon: {
        ...formData.taxon,
        clados: (formData.taxon.clados || []).filter((val) => val.trim() !== ''),
      },
    };
    ...
    if (includeImagenes) {
      const payloadImagenes: PlantDictionary = {
        flor: formData.imagenes?.flor || { src: '', alt: '' },
        hojas: formData.imagenes?.hojas || { src: '', alt: '' },
      };
    ...
    try {
      const response = await upsertSpecimen({ data: payloadToSave });
      ...
      if (response.success || (response.data && response.failed === 'file')) {
        ...
        setIncludeImagenes(!!savedData.imagenes);

        if (response.failed === 'file') {
          console.warn('Note: Database updated, but JSON backup failed:', response.errors?.file);
        }
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      if (response.success || response.failed === 'file') {
        ...
        console.error('Failed to delete. Point of failure:', response.failed, response.errors);
    ...
    <>
      <form className={styles.formContainer} onSubmit={handleSubmit}>
        ...
        <BasicInfoSection
          ...
          onChange={(field, value) => updateFormDataField(field, value)}
        />

        <DynamicStringArray
          title="Nombres Comunes"
          items={formData.nombresComunes || []}
          onChange={(newItems) => updateFormDataField('nombresComunes', newItems)}
        />
        <DynamicStringArray
          title="Partes Útiles"
          items={formData.partesUtiles || []}
          onChange={(newItems) => updateFormDataField('partesUtiles', newItems)}
        />
        <DynamicStringArray
          title="Propiedades Medicinales"
          items={formData.propiedadesMedicinales || []}
          onChange={(newItems) => updateFormDataField('propiedadesMedicinales', newItems)}
        />
        <DynamicStringArray
          title="Qué males emocionales cura"
          items={formData.malesEmocionales || []}
          onChange={(newItems) => updateFormDataField('malesEmocionales', newItems)}
        />
        <DynamicStringArray
          title="Que males físicos cura"
          items={formData.malesFisicos || []}
          onChange={(newItems) => updateFormDataField('malesFisicos', newItems)}
        />
        <DynamicStringArray
          title="Esencias Florales"
          items={formData.esenciasFlorales || []}
          onChange={(newItems) => updateFormDataField('esenciasFlorales', newItems)}
        />
        ...
        <ImageGallerySection
          ...
          onChange={(newImagenes) => updateFormDataField('imagenes', newImagenes)}
        />
        ...
        <div className={styles.section}>
          ...
          <DynamicStringArray
            ...
            onChange={(newItems) => updateFormDataField('correspondenciasEnergeticas', newItems)}
          />
        ...
        <TaxonomySection
          ...
          onChange={(newTaxon) => updateFormDataField('taxon', newTaxon)}
        />

        <PreparationsSection
          preparaciones={formData.preparaciones || []}
          onChange={(newPreps) => updateFormDataField('preparaciones', newPreps)}
        />
        ...
        <div style={{ display: 'flex', gap: '1rem', marginTop: '2rem' }}>
          ...
          <button
            ...
            disabled={isProcessing}
          >
            {isProcessing ? 'Procesando el salvado...' : 'Guardar Espécimen'}
          </button>
        </div>
      </form>
