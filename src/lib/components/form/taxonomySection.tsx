...
export default function TaxonomySection({ taxon, onChange }: TaxonomySectionProps) {
  ...
  const removeClado = (indexToRemove: number) => {
    onChange({
      ...taxon,
      clados: (taxon.clados || []).filter((_, index) => index !== indexToRemove),
    });
  };
  ...
  return (
    <div className={styles.section}>
      ...
      <div className={styles.row}>
        {['dominio', 'reino', 'filo', 'clase', 'orden', 'familia', 'genero', 'especie'].map((taxRank) => (
          <div
            key={taxRank}
          ...
          >
            ...
            <input
              ...
              onChange={(e) => handleTaxonChange(taxRank as keyof Taxon, e.target.value)}
            />
          </div>
        ))}
      </div>
