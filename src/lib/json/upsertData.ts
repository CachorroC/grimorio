...
import { arrayedMapPlantList } from './gemini_plantList';
  ...
  for (const item of combined) {
    if (typeof item === 'string') {
      ...
      if (!uniqueMap.has(normalized)) {
        ...
        uniqueMap.set(normalized, item.trim());
      }
    }
  }
...
function getDeterministicHash(obj: any): string {
  ...
  if (Array.isArray(obj)) {
    return `[${obj.map(getDeterministicHash).join(',')}]`;
  }
  ...
  if (typeof obj === 'object') {
    ...
    const parts = keys.map((key) => {
      return `"${key}":${getDeterministicHash(obj[key])}`;
    });
...
/**
 * Generic helper function to merge arrays of complex objects without exact duplicates.
 * Uses deterministic hashing to ensure identical objects aren't duplicated regardless of key order.
 */
function mergeComplexArrays<T>(arr1?: T[] | null, arr2?: T[] | null): T[] {
  const combined = [...(arr1 || []), ...(arr2 || [])];
...
export async function upsertEspecimenesConMerge(
  ...
  const nombresCientificos = nuevosEspecimenes.map((e) => {
  ...
  for (const doc of existingDocs) {
    existingMap.set(doc.nombreCientifico, doc);
  }

  // 4. Prepare the BulkWrite operations array
  const operations: AnyBulkWriteOperation<EspecimenType>[] =
    nuevosEspecimenes.map((incoming) => {
      const existing = existingMap.get(incoming.nombreCientifico);
      ...
      if (existing) {
        ...
        const mergedTaxon: Taxon = {
          ...
          clados: mergeStringArrays(
            existing.taxon?.clados,
            incoming.taxon?.clados,
          ),
        };

        // Handle the tuple casting for polaridadEnergetica safely
        const mergedPolaridad = mergeStringArrays(
          existing.polaridadEnergetica as string[],
          incoming.polaridadEnergetica as string[],
        );
        ...
        const mergedDoc: EspecimenType = {
          ...
          nombresComunes: mergeStringArrays(
            existing.nombresComunes,
            incoming.nombresComunes,
          ),
          propiedadesMedicinales: mergeStringArrays(
            existing.propiedadesMedicinales,
            incoming.propiedadesMedicinales,
          ),
          partesUtiles: mergeStringArrays(
            existing.partesUtiles,
            incoming.partesUtiles,
          ),
          esenciasFlorales: mergeStringArrays(
            existing.esenciasFlorales,
            incoming.esenciasFlorales,
          ),
          correspondenciasEnergeticas: mergeStringArrays(
            existing.correspondenciasEnergeticas,
            incoming.correspondenciasEnergeticas,
          ),
          malesEmocionales: mergeStringArrays(
            existing.malesEmocionales,
            incoming.malesEmocionales,
          ),
          malesFisicos: mergeStringArrays(
            existing.malesFisicos,
            incoming.malesFisicos,
          ),

          taxon: mergedTaxon,

          // Reused generic merger for complex object arrays
          preparaciones: mergeComplexArrays<PreparacionType>(
            existing.preparaciones,
            incoming.preparaciones,
          ),
          chakrasAsociados: mergeComplexArrays<ChakraType>(
            existing.chakrasAsociados,
            incoming.chakrasAsociados,
          ),

          // ElementosType is a string union, fallback to incoming if existing is missing
          elementosAsociados:
            existing.elementosAsociados || incoming.elementosAsociados,

          // Use the merged strings casted back to the expected Tuple type
...
export async function executeUpsert() {
  ...
  try {
    ...
    const dataEntrante = arrayedMapPlantList;
    ...
    await upsertEspecimenesConMerge(collection, dataEntrante);

    console.log('✅ Upsert y merge completados con éxito.');
  } catch (error) {
