...
async function upsertSpecimenToDB({ data }: { data: EspecimenType }) {
  try {
...
async function upsertSpecimenToJSON({ data }: { data: EspecimenType }) {
  try {
    ...
    const { _id: _, ...jsonSafeData } = data as any;
...
export async function upsertSpecimen({ data }: { data: EspecimenType }) {
  ...
  if (dbResult.success && fileResult.success) {
    revalidatePath('/hierbas', 'layout');
    revalidateTag('hierbas', 'max');
  ...
  return {
    ...
    failed,
    errors,
    data: dbResult.success ? dbResult.data : undefined,
  };
}
...
async function deleteSpecimenFromDB({
  ...
  try {
    ...
    const result = await specimens.deleteOne(query);

    if (result.deletedCount === 0) {
      throw new Error('No document found to delete in MongoDB.');
    }
...
async function deleteSpecimenFromJSON({
  ...
  try {
    ...
    const fileContents = await fs.readFile(jsonFilePath, 'utf8');
    let plantList = JSON.parse(fileContents) as EspecimenType[];
    ...
    if (plantList.length === initialLength) {
      throw new Error('No document found to delete in JSON.');
    }
